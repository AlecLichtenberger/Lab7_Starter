describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 0; i<prodItemsData.length; i++){
      ...
      currProdValue = prodItemsData[i];
      if (currProdValue.title.length == 0) { allArePopulated = false; }
      if (currProdValue.price.length == 0) { allArePopulated = false; }
      if (currProdValue.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    const buttonTexts4 = await page.$$eval('product-item', items =>
      items.map(item => item.shadowRoot.querySelector('button').innerText)
    );
    for (const text of buttonTexts4) {
      expect(text).toBe('Remove from Cart');
    }
    const cartCount4 = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCount4).toBe('20');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCount6 = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCount6).toBe('0');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    const buttonTexts7 = await page.$$eval('product-item', items =>
      items.map(item => item.shadowRoot.querySelector('button').innerText)
    );
    for (const text of buttonTexts7) {
      expect(text).toBe('Add to Cart');
    }
    const cartCount7 = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCount7).toBe('0');
